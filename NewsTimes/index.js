...
const API_KEY = "kMNwfzPPNCLvwAE5itr23JzQPhMNc2JLlcurCqxNsZo";
let articles = [];
let page = 1;
let totalPage = 1;
let url = new URL(
...
const getNews = async () => {
  try {
    ...
    url.searchParams.set("page", page); // 8.page
...
const renderPagenation = () => {
  ...
  // 3.만약에 first가 6 이상이면 prev 버튼 담 
  // 4.만약에 last가 마지막이 아니라면 next버튼을 담
  // 5.마지막이 5개이하이면 last=totalpage
  // 6.페이지가 5개 이하라면 first = 1
